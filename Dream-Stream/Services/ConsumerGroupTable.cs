...
namespace Dream_Stream.Services
{
    public class ConsumerGroupTable
    {
        ...
        public static Dictionary<string, List<int>> GetPartitionDistributionList(int partitionCount, List<string> consumerIds)
        {
            ...
            var partitionsPrConsumerFloored = partitionCount / consumerIds.Count;
            var extraPartitions = partitionCount % consumerIds.Count;
            ...
            for (var partition = 0; partition < partitionCount; partition++)
            {
                ...
                if (++currentPartitionCount < partitionsPrConsumerFloored || (currentPartitionCount == partitionsPrConsumerFloored && consumerIdIndex < extraPartitions)) continue;
                consumerIdIndex++;
